...
from baseline_models.channel_enc_dec_blocks import ChannelEncoder, ChannelDecoder
import torch
...
class Tx_Relay_Rx(nn.Module):
    def __init__(self, nin, n_emb, n_latent, tx_rx_channel: Channel, relay_rx_channel: Channel,
        ...
        self.tx_rx_channel = tx_rx_channel
        self.relay_rx_channel = relay_rx_channel

    def forward(self, x, attention_mask):
        x_hat, x1 = self.tx_relay_model(x, attention_mask)

        x_hard = torch.argmax(x_hat, dim=2)

        x_emb = self.relay_embedding(x_hard)

        y2 = self.relay_rx_channel(self.relay_encoder(x_emb))
        y1 = self.tx_rx_channel(x1)
