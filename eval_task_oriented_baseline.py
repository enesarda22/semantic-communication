...
def main():
    ...
    for di, d_sd in enumerate(d_list):
        for gi, gamma in enumerate(gamma_list):
            ...
            for b in tqdm(data_handler.test_dataloader, desc="Generating", leave=False):
                ...
                predicted_ids = (torch.argmax(logits, dim=-1)).reshape(B, args.max_length)
            ...
            row: Dict[str, Any] = {
                "baseline_tx_relay_path": args.tx_relay_path,
                "baseline_tx_relay_rx_path": args.tx_relay_rx_path,
    ...
    summary_path = os.path.join(args.results_dir, "baseline_task_summary.csv")
    ...
    if args.save_examples:
        examples_path = os.path.join(args.results_dir, "baseline_task_examples.csv")
