...
if __name__ == "__main__":
    ...
    for distance_index, d_sr in enumerate(args.d_list):
        ...
        np.save(os.path.join(results_dir, f"ae_conventional_mean_semantic_sim_{args.channel_type}.npy"), mean_semantic_sim)
        np.save(os.path.join(results_dir, f"ae_conventional_mean_sbert_semantic_sim_{args.channel_type}.npy"), mean_sbert_semantic_sim)
        np.save(os.path.join(results_dir, f"ae_conventional_mean_bleu_1_{args.channel_type}.npy"), mean_bleu_1)
        np.save(os.path.join(results_dir, f"ae_conventional_mean_bleu_{args.channel_type}.npy"), mean_bleu)

        np.save(os.path.join(results_dir, f"ae_conventional_std_semantic_sim_{args.channel_type}.npy"), std_semantic_sim)
        np.save(os.path.join(results_dir, f"ae_conventional_std_sbert_semantic_sim_{args.channel_type}.npy"), std_sbert_semantic_sim)
        np.save(os.path.join(results_dir, f"ae_conventional_std_bleu_1_{args.channel_type}.npy"), std_bleu_1)
